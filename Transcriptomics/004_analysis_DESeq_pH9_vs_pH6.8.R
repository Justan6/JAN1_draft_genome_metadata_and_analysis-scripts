...
# Keep only pH 9.0 and 6.8 samples
keep_samples <- meta$pH %in% c("9.0", "6.8")
...
# ==========================
# 7. COMPARE pH 9.0 vs 6.8
# ==========================
res_9vs68 <- results(dds, contrast = c("pH", "9.0", "6.8"))
res_9vs68 <- res_9vs68[order(res_9vs68$pvalue), ]  # sort by p-value


cat("\n--- SUMMARY ---\n")
cat("Total genes analyzed:\t", nrow(res_9vs68), "\n")
cat("Significant DEGs (padj < 0.001):\t", sum(res_9vs68$padj < 0.001, na.rm = TRUE), "\n")
cat("Upregulated (log2FC > 1, padj < 0.001):\t", sum(res_9vs68$log2FoldChange > 1 & res_9vs68$padj < 0.001, na.rm = TRUE), "\n")
cat("Downregulated (log2FC < -1, padj < 0.001):\t", sum(res_9vs68$log2FoldChange < -1 & res_9vs68$padj < 0.001, na.rm = TRUE), "\n")


# ==========================
# 8. SAVE RESULTS
# ==========================
#results_file <- "results/DEGs/pH9_vs_pH68.csv"
dir.create("results/DEGs", showWarnings = FALSE, recursive = TRUE)
#write.csv(as.data.frame(res_9vs68), file = results_file)
...
cat("\n--- SUMMARY ---\n")
cat("Total genes analyzed:\t", nrow(res_9vs68), "\n")
cat("Significant DEGs (padj < 0.05):\t", sum(res_9vs68$padj < 0.05, na.rm = TRUE), "\n")
cat("Upregulated (log2FC > 1, padj < 0.05):\t", sum(res_9vs68$log2FoldChange > 1 & res_9vs68$padj < 0.05, na.rm = TRUE), "\n")
cat("Downregulated (log2FC < -1, padj < 0.05):\t", sum(res_9vs68$log2FoldChange < -1 & res_9vs68$padj < 0.05, na.rm = TRUE), "\n")
...
# ==========================================================
res_df <- as.data.frame(res_9vs68)
...
#write.csv(top_degs_annotated, file = "results/DEGs/pH9_vs_pH68_top_DEGs_annotated.csv", row.names = FALSE)
#cat("✅ Annotated DEG results saved to: results/DEGs/pH9_vs_pH68_top_DEGs_annotated.csv\n")
...
meta <- data.frame(sample = sample_names, pH = factor(pH_condition, levels = c("9.0", "6.8")))
rownames(meta) <- meta$sample
...
write.csv(degs_tpm,
          file = "results/DEGs/pH9_vs_pH68_TPM_log2TPM_by_condition.csv",
          row.names = FALSE)
cat("✅ Full TPM/log2TPM by condition saved to results/DEGs/pH9_vs_pH68_TPM_log2TPM_by_condition.csv\n")
