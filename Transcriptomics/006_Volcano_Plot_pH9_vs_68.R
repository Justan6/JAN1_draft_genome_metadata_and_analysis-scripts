...
data <- read_csv("results/DEGs/pH9_vs_pH68_TPM_log2TPM_by_condition.csv")
...
data_plot <- data %>%
  ...
  mutate(
    DE_status = case_when(
      log2FoldChange >= LOG2FC_THRESHOLD & padj < P_ADJ_THRESHOLD ~ "Up-regulated (pH 9.0)",
...
volcano_plot <- ggplot(data_plot, aes(x = log2FoldChange, y = neg_log10_padj)) +
  ...
  scale_color_manual(
    values = c(
      "Up-regulated (pH 9.0)" = "#1F78B4",     # Blue
  ...
  labs(
    title = "Volcano Plot: Differential Gene Expression (pH 9.0 vs pH 6.8)",
    x = expression(log[2]~"Fold Change"~"(pH 9.0 / pH 6.8)"),
...
ggsave("results/figures/Volcano_plot/2.Volcano_Plot_9.0.0vs6.8_Msumavensis_labeled.png", plot = volcano_plot, width = 10, height = 8, units = "in", dpi = 300)
ggsave("results/figures/Volcano_plot/2.Volcano_Plot_9.0.0vs6.8_Msumavensis_labeled.svg", plot = volcano_plot, width = 10, height = 8, units = "in", dpi = 300)
ggsave("results/figures/Volcano_plot/2.Volcano_Plot_9.0.0vs6.8_Msumavensis_labeled.pdf", plot = volcano_plot, width = 10, height = 8, units = "in", dpi = 300)
