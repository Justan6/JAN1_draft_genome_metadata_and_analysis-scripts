...
deg_data <- read_ods("results/DEGs/Functional_Categories.ods", sheet = "Selected_pH_5.0_vs_pH6.8_Functional_Categories")
...
df_plot <- deg_data %>%
  ...
  mutate(Direction = case_when(
    log2FoldChange > 1  ~ "Up-regulated (pH 5.0)",
    log2FoldChange < -1 ~ "Down-regulated (pH 5.0)",
...
plot_genes <- ggplot(df_plot, aes(x = log2FoldChange, y = product_factor, fill = Direction)) +
  geom_col(width = 0.8) +
  scale_fill_manual(values = c("Up-regulated (pH 5.0)" = "blue", 
                               "Down-regulated (pH 5.0)" = "red")) +
  ...
  labs(
    ...
    y = NULL,
    title = "Differentially Expressed Genes by COG Functional Category (pH 5.0 vs pH 6.8)"
  ) +
...
ggsave(file.path(fig_dir, "pH5.0_vs_pH6.8_DEGs_barplot_shaded.pdf"), plot_genes, width = 15, height = 30)
ggsave(file.path(fig_dir, "pH5.0_vs_pH6.8_DEGs_barplot_shaded.svg"), plot_genes, width = 15, height = 30)
ggsave(file.path(fig_dir, "pH5.0_vs_pH6.8_DEGs_barplot_shaded.png"), plot_genes, width = 15, height = 30)
